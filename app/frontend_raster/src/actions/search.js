...
export const fetchSearchResults = (searchtext) => {
  return (dispatch, getState) => {
    ...
    let searchJSON = {searchtext: searchtext};
    if (map) {
      let center = map.getCenter();    
      searchJSON = {searchtext: searchtext, lat: center.lat, lng: center.lng};
    }
    let context = getState().global.context;
    if (context) searchJSON['context'] = context.shortcode;
    let body = JSON.stringify(searchJSON);
