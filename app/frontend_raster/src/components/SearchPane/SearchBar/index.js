...
export class SearchBar extends Component {
  ...
  selectItem = (item) => {
    ...
    switch(item['type']) {
      ...
      case 'location':
        ...
        this.props.global.map.flyTo({lng: item.lng, lat: item.lat}, item.zoom, {animate: false});
        break;  
      case 'context':
        this.props.resetGeosearch();
        this.props.setGlobalState({'drawer': false});
        const southWest = [item.bounds[1], item.bounds[0]]
        const northEast = [item.bounds[3], item.bounds[2]]
        this.props.global.map.fitBounds([southWest, northEast], {
          paddingBottomRight: this.props.isMobile ? [0, 0] : DESKTOP_PADDING,
          animate: true,
        });  
        break;  
      case 'selection':
        ...
        this.props.fetchEntitiesByProperty(item['id'], this.props.isMobile).then(() => {
          this.props.redrawGeoJSON();
        });    
        break;
      default:
        this.props.resetGeosearch();
        this.props.setGlobalState({'drawer': true});
        this.props.fetchEntity(item['id'], this.props.isMobile).then(() => {
          this.props.redrawGeoJSON();
        });    
        break;  
      }
  ...
  gotoCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      this.props.global.map.flyTo({lng: position.coords.longitude, lat: position.coords.latitude}, 14, {animate: true});
        ...
        <div className="search-bar-container">
            ...
            {!this.props.global.context ? (
              <IonIcon
                ...
                style={{ fontSize: 24, paddingLeft: "8px", cursor: "pointer" }}
              />
            ) : null}
            <input 
              id="searchinput"
...
export default withSizes(mapSizesToProps)(connect(mapStateToProps, mapDispatchToProps)(SearchBar));